...
public class OpmZarrRealDataTest {
	...
	public void opensProductionDatasetVirtually() {
		...
		OpmZarrView.Options volumeOptions = new OpmZarrView.Options();
		volumeOptions.tryGpu = false;
		volumeOptions.requestedChannels.add(dataset.getChannelLabels().get(0));
		ImagePlus volume = OpmZarrView.openVirtualVolume(dataset, volumeOptions, 0);
		...
		if (Boolean.getBoolean("opm.test.gpu")) {
			OpmZarrPlaneReader reader = new OpmZarrPlaneReader(dataset, "s0");
			try {
