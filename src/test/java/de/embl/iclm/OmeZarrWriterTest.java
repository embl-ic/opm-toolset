...
public class OmeZarrWriterTest {
	...
	public void writesCanonicalDatasetAndN5ReadsIt() throws Exception {
		...
		writer.writeMetadata(provenance, OmeZarrSession.PROJECTIONS);
		...
		assertEquals(2, attrs.getAsJsonObject(OmeZarrWriter.WRITE_STATE_KEY)
		...
		assertTrue(new File(root, OmeZarrWriter.SUCCESS_FILE).isFile());
	...
	public void refusesIncompleteCommitAndRecoversStagedTimepoint() throws Exception {
		...
		OmeZarrWriter first = new OmeZarrWriter(root);
		...
		OmeZarrWriter resumed = new OmeZarrWriter(root);
		...
		OmeZarrWriter reopened = new OmeZarrWriter(root);
		...
		assertFalse("interrupted/resumed output is not complete until normal finalization",
				new File(root, OmeZarrWriter.SUCCESS_FILE).exists());
		volume.close();
		projection.close();
	}
	...
	@Test
	public void migratesTheFormerAnisotropicDeskewScaleInAllMetadata() throws Exception {
		File root = new File(folder.getRoot(), "legacy-scale.ome.zarr");
		OpmProvenance provenance = new OpmProvenance();
		provenance.datasetName = "legacy-scale";
		...
		JsonObject attrs = json(new File(root, ".zattrs"));
		JsonArray storedVoxel = attrs.getAsJsonObject(OpmProvenance.KEY)
				.getAsJsonArray("deskewedVoxelSizeUm");
		assertEquals(0.116, storedVoxel.get(0).getAsDouble(), 0);
		assertEquals(0.116, storedVoxel.get(1).getAsDouble(), 0);
		assertEquals(0.116, storedVoxel.get(2).getAsDouble(), 0);
		JsonArray volumeScale = attrs.getAsJsonArray("multiscales").get(0).getAsJsonObject()
		...
		assertEquals(30.0, volumeScale.get(0).getAsDouble(), 0);
		assertEquals(0.116, volumeScale.get(2).getAsDouble(), 0);
		assertEquals(0.116, volumeScale.get(3).getAsDouble(), 0);
		assertEquals(0.116, volumeScale.get(4).getAsDouble(), 0);
		assertProjectionCalibration(attrs, "maxX", "y", "z", 0.116, 0.116);
	}

	private static void createAllProjectionArrays(
			OmeZarrWriter writer, int width, int height, int depth, int channels) throws IOException {
	...
	private static void writeAllProjections(
			OmeZarrWriter writer, ImagePlus volume, int channel, int time) throws IOException {
		for (String name : OmeZarrSession.PROJECTIONS) {
			char axis = name.charAt(name.length() - 1);
