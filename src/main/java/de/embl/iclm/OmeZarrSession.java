...
public final class OmeZarrSession implements AutoCloseable {
	...
	private final OmeZarrWriter writer;
	private OpmProvenance provenance;

	/**
	 * How long one appended time point spent in each stage.
	 *
	 * <p>An OME-Zarr run reported nothing but "committed", so there was no way to tell a slow
	 * GPU from a slow disk, or to see the cost of the projections. The three stages are
	 * measured where they actually happen: deskew is whatever elapses outside the sink,
	 * projections and chunk writing are timed inside it.
	 */
	public static final class Timing {
		public double deskewSec;
		public double projectionSec;
		public double writeSec;
		public double totalSec;
		public int channelCount;

		/** One line for a log: the stages, then the total. */
		public String format() {
			return String.format(java.util.Locale.US,
					"%d channel(s): deskew %.3f, projections %.3f, write %.3f, total %.3f s",
					Integer.valueOf(channelCount), Double.valueOf(deskewSec),
					Double.valueOf(projectionSec), Double.valueOf(writeSec), Double.valueOf(totalSec));
		}
	}
	...
	public OmeZarrSession(
		...
		writer = new OmeZarrWriter(zarrRoot);
		OpmProvenance existing = writer.getProvenance();
		if (existing != null) {
			validateStableMetadata(existing, requestedProvenance);
			// This field was added after schema 2 was first written. Merge it into a compatible
			// resumed dataset without changing the matrix or any processing decision.
			if ((existing.alignMatrixModifiedUtc == null || existing.alignMatrixModifiedUtc.trim().isEmpty())
					&& requestedProvenance != null)
				existing.alignMatrixModifiedUtc = requestedProvenance.alignMatrixModifiedUtc;
			/* The channel layout arrived later still, and is display metadata only: a store
			 * that has none takes the resuming run's, one that has one keeps it. */
			if (existing.deskewChannelOption == null && !existing.deskewCombineChannels
					&& requestedProvenance != null) {
				existing.deskewChannelOption = requestedProvenance.deskewChannelOption;
				existing.deskewCombineChannels = requestedProvenance.deskewCombineChannels;
				existing.deskewChannelOrder = new java.util.ArrayList<String>();
				if (requestedProvenance.deskewChannelOrder != null)
					existing.deskewChannelOrder.addAll(requestedProvenance.deskewChannelOrder);
			}
			provenance = existing;
		} else {
	...
	public synchronized boolean append(OpmTimepointProcessor.TimePoint timePoint) throws IOException {
		return append(timePoint, null);
	}

	/**
	 * Process and append a complete raw time point, recording how long each stage took.
	 *
	 * @param timing when not null, filled in with the stage durations of this append
	 * @return true when a new T position was committed, false when its label was already present
	 */
	public synchronized boolean append(
			OpmTimepointProcessor.TimePoint timePoint, final Timing timing) throws IOException {
		...
		// everything not spent inside the sink was spent reading and deskewing
		final long[] insideSink = { 0 };
		try {
			...
			OpmTimepointProcessor.processChannels(timePoint, deskewMatrix, tryGpu,
				...
				public void accept(ImagePlus volume, String label, int channel, boolean usedGpu)
					...
					try {
						...
						if (timing != null) timing.channelCount++;
					} finally {
						insideSink[0] += System.nanoTime() - sinkStart;
					}
				}
			});
			...
			if (timing != null) {
				...
				timing.totalSec = total / 1.0e9;
				timing.deskewSec = Math.max(0, total - insideSink[0]) / 1.0e9;
			}
		}
	}

	private static void add(Timing timing, String stage, long elapsedNanos) {
		if (timing == null) return;
		double seconds = elapsedNanos / 1.0e9;
		if ("write".equals(stage)) timing.writeSec += seconds;
		else timing.projectionSec += seconds;
	}
	...
	private static void validateStableMetadata(OpmProvenance existing, OpmProvenance requested) throws IOException {
		...
		if (!same(existing.xyPixelSizeUm, requested.xyPixelSizeUm)
				...
				|| !matrixMapsEqual(existing.alignMatrices, requested.alignMatrices)
				|| !equal(existing.alignReference, requested.alignReference)
				|| existing.alignApplied != requested.alignApplied
				/* alignFlipHalf and alignInterpolate are deliberately not compared. They name how a
				 * viewer shows the store - which side it flips, how it samples a transformed half -
				 * and neither reaches a stored pixel: the halves are unflipped and unaligned and the
				 * matrices are measured right-mirrored whatever is chosen. Resuming with either
				 * changed appends exactly the same pixels; the store keeps what it began with. */
	...
	private static boolean matrixMapsEqual(java.util.Map<String, double[][]> a,
			java.util.Map<String, double[][]> b) {
		if (a == b) return true;
		if (a == null || a.isEmpty()) return b == null || b.isEmpty();
		if (b == null || a.size() != b.size() || !a.keySet().equals(b.keySet())) return false;
		for (String key : a.keySet()) if (!matricesEqual(a.get(key), b.get(key))) return false;
		return true;
	}
